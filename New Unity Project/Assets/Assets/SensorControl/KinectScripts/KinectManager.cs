...
//	public bool NearMode = false;
    ...
    private List<UnitController> Player1Controllers;
    private List<UnitController> Player2Controllers;
...
//				break;
    ...
    // recreates and reinitializes the lists of avatar controllers, after the list of avatars for player 1/2 was changed
    public void ResetUnitControllers()
    {
        if (Player1Units.Count == 0 && Player2Units.Count == 0)
        {
            UnitController[] avatars = FindObjectsOfType(typeof(UnitController)) as UnitController[];

            foreach (UnitController unit in avatars)
            {
                Player1Units.Add(unit.gameObject);
            }
        }
	...
	{
		if (!isLocalPlayer) {
			gameObject.GetComponent<KinectManager> ().enabled=false;
			return;
		}
        ...
        // Pull the AvatarController from each of the players Avatars.
        Player1Controllers = new List<UnitController>();
        Player2Controllers = new List<UnitController>();
        ...
        // Add each of the avatars' controllers into a list for each player.
        foreach (GameObject avatar in Player1Units)
        {
            if (avatar != null && avatar.activeInHierarchy)
            {
                Player1Controllers.Add(avatar.GetComponent<UnitController>());
            }
        }

        foreach (GameObject avatar in Player2Units)
        {
            if (avatar != null && avatar.activeInHierarchy)
            {
                Player2Controllers.Add(avatar.GetComponent<UnitController>());
            }
        }

        // create the list of gesture listeners
        gestureListeners = new List<KinectGestures.GestureListenerInterface>();
		//본인의 카메라를 initialization 당시에 제스쳐리스너로 등록.
		gestureListeners.Add ((KinectGestures.GestureListenerInterface)transform.Find ("Body").Find ("Stoma").Find ("Main Camera").GetComponent<GestureListener>());
//		foreach(MonoBehaviour script in GestureListeners)
//		{
//			// Debug.Log("script is KinectGestures.GestureListenerInterface ? : " + (script is KinectGestures.GestureListenerInterface));
//			if(script && (script is KinectGestures.GestureListenerInterface))
//			{
//				KinectGestures.GestureListenerInterface listener = (KinectGestures.GestureListenerInterface)script;
//				gestureListeners.Add(listener);
//				// Debug.Log("gestureListeners.Add(listener)");
//			}
//		}
	...
	void Update()
	{
        ...
        if (KinectInitialized)
        {
            ...
            if (KinectWrapper.PollSkeleton(ref smoothParameters, ref skeletonFrame))
            {
                ...
                //Debug.Log("ProcessSkeleton Not Called");
            }
            ...
            if (Player1Calibrated)
            {
                foreach (UnitController controller in Player1Controllers)
                {
                    //if(controller.Active)
                    {
                        controller.UpdateUnit(Player1ID);
                    }
                }
				...
				{
					// Debug.Log ("Check for complete gestures");
					...
					if(gestureData.complete)
					{
						...
						foreach(KinectGestures.GestureListenerInterface listener in gestureListeners)
						{
							...
							{
								// Debug.Log ("ResetPlayerGestures");
								ResetPlayerGestures(Player1ID);
							}
						}
					}
					else if(gestureData.cancelled) // Case cancelled
					{
						foreach(KinectGestures.GestureListenerInterface listener in gestureListeners)
						{
					...
					else if(gestureData.progress >= 0.1f)
					{
						...
						foreach(KinectGestures.GestureListenerInterface listener in gestureListeners)
						{
							// Debug.Log("KinectGestures.GestureListenerInterface listener in gestureListeners)");
            ...
            {
                foreach (UnitController controller in Player2Controllers)
                {
                    //if(controller.Active)
                    {
                        controller.UpdateUnit(Player2ID);
                    }
                }
...
//					}
		...
		if(!Player1Calibrated)
		{
			...
			if (!allUsers.Contains(UserId))
			{
				if(CheckForCalibrationPose(UserId, ref Player1CalibrationPose, ref player1CalibrationData, ref skeletonData))
				{
					...
					Debug.Log ("add the gestures to detect, if any");
	...
	void ProcessSkeleton()
	{
		...
		for(int i = 0; i < KinectWrapper.Constants.NuiSkeletonCount; i++)
		{
            ...
            // Debug.Log("skeletonData.eTrackingState: " + skeletonData.eTrackingState.ToString() );
