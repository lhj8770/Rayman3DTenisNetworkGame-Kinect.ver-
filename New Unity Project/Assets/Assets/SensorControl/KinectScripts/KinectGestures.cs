...
public class KinectGestures
{
    ...
    private const int SpineIndex = (int)KinectWrapper.NuiSkeletonPositionIndex.Spine;
...
//								(jointsPos[leftHandIndex].x - gestureData.jointPos.x) > 0.15f;
			...
			// check for SwingForward
		case  Gestures.SwingForward:
				switch (gestureData.state) {
					case 0:
					if (    jointsTracked[rightHandIndex] && jointsTracked[rightShoulderIndex] && jointsTracked[hipCenterIndex] && jointsTracked[shoulderCenterIndex] &&
                            jointsPos [rightHandIndex].z >= jointsPos [rightShoulderIndex].z &&
						    jointsPos [rightHandIndex].x >= jointsPos [hipCenterIndex].x &&
							jointsPos [rightHandIndex].y <= ( jointsPos[hipCenterIndex].y + jointsPos[shoulderCenterIndex].y ) / 2.0f  ) {

							// 제스쳐 데이터에 joint information을 기록한다.
							SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
							gestureData.progress = 0.5f;
							Debug.Log ("SwingForward 준비완료");
						} else {
							// cancel the gesture
							SetGestureCancelled(ref gestureData);
						}
					break;
					case 1:
						if( (timestamp - gestureData.timestamp) < 1.2f)
						{
						
							//
							bool isMotionZ = jointsPos [rightHandIndex].z <= (jointsPos [hipCenterIndex].z + jointsPos [leftHipIndex].z + jointsPos [rightHipIndex].z) / 3.0f
                                    && jointsTracked[rightHandIndex] && jointsTracked[hipCenterIndex] && jointsTracked[leftHipIndex] && jointsTracked[rightHipIndex];
                            // bool isMotionX = jointsPos [rightHandIndex].x <= (jointsPos [hipCenterIndex].x + jointsPos [rightHipIndex].x) / 2.0f;
                            // bool isMotionX = jointsPos [rightHandIndex].x <= jointsPos [rightElbowIndex].x ;
                            bool isMotionY = jointsPos [rightHandIndex].y >= jointsPos [rightHipIndex].y
                                && jointsTracked[rightHandIndex] && jointsTracked[rightHipIndex];
							bool isInPose = isMotionZ && isMotionY ;

							string condition = string.Format ("{0} {1}", isMotionZ, isMotionY );
							Debug.Log ("SwingForward 계산 : " + condition);

							if (isInPose) {
							Debug.Log ("SwingForward 완료");
								Vector3 jointPos = jointsPos [gestureData.joint];
								CheckPoseComplete (ref gestureData, timestamp, jointPos, isInPose, 0f);
							} else {
		...
		case  Gestures.SwingBackward:
			switch (gestureData.state) {
			case 0:
				if (jointsTracked[rightHandIndex] && jointsTracked[rightShoulderIndex] && jointsTracked[hipCenterIndex] && jointsTracked[shoulderCenterIndex] && jointsTracked[leftHipIndex] && jointsTracked[rightHipIndex] &&
                    jointsPos [rightHandIndex].z <= jointsPos [rightShoulderIndex].z &&  // z축 오른쪽 골반보다 앞에 손목이 위치할 때
					jointsPos [rightHandIndex].x <= (jointsPos [hipCenterIndex].x+jointsPos [leftHipIndex].x)/2.0f &&		// x축  '중심과 오른쪽골반 중심'보다 손목이 전에 위치할때
					jointsPos [rightHandIndex].y >= ( ( jointsPos[hipCenterIndex].y + jointsPos[leftHipIndex].y + jointsPos[rightHipIndex].y ) / 3.0f)
					&& jointsPos[rightHandIndex].y <= (jointsPos[hipCenterIndex].y+jointsPos[shoulderCenterIndex].y)/2.0f )  // y축 3개의 힙의 평균높이보다 높을 때 
				{
					// 제스쳐 데이터에 joint information을 기록한다.
					SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
					gestureData.progress = 0.5f;
					Debug.Log ("SwingBackward 준비완료");
				} else {
					// cancel the gesture
					SetGestureCancelled(ref gestureData);
				}
			...
			case 1:
				if( (timestamp - gestureData.timestamp) < 1.2f){
					...
					// bool isMotionZ = jointsPos [rightHandIndex].z <= (jointsPos [hipCenterIndex].z + jointsPos [leftHipIndex].z + jointsPos [rightHipIndex].z) / 3.0f ;
					bool isMotionZ = jointsPos [rightHandIndex].z <= jointsPos [leftHipIndex].z && jointsPos [rightHandIndex].z <= jointsPos [rightHipIndex].z
                                && jointsTracked[rightHandIndex] && jointsTracked[leftHipIndex] && jointsTracked[rightHandIndex] && jointsTracked[rightHipIndex];
					bool isMotionX = jointsPos [rightHandIndex].x >= jointsPos [rightHipIndex].x
                                && jointsTracked[rightHandIndex] && jointsTracked[rightHipIndex] ;

					bool isInPose = isMotionZ && isMotionX;
						// && jointsPos [rightHandIndex].y >= ((jointsPos [hipCenterIndex].y + jointsPos [shoulderCenterIndex].y) / 3.0f);

					string condition = string.Format ("{0} {1}", isMotionZ, isMotionX );
					Debug.Log ("SwingBackward 계산 : " + condition);

					if (isInPose) {
						Debug.Log ("SwingBackward 완료");
						Vector3 jointPos = jointsPos [gestureData.joint];
						CheckPoseComplete (ref gestureData, timestamp, jointPos, isInPose, 0f);
					} else {
			...
			break;
            case Gestures.SwingTopdown:
                switch (gestureData.state)
                {
                    case 0:
                        if (jointsTracked[rightHandIndex] && jointsTracked[HeadIndex] && jointsTracked[leftShoulderIndex] &&
                            jointsPos[rightHandIndex].y >= jointsPos[HeadIndex].y &&  // [높이] 오른손목 >= 얼굴
                            jointsPos[rightHandIndex].x >= jointsPos[leftShoulderIndex].x      // [x축] 오른손목 >= 왼쪽어깨
                           )  // y축 3개의 힙의 평균높이보다 높을 때 
                        {
                            // 제스쳐 데이터에 joint information을 기록한다.
                            SetGestureJoint(ref gestureData, timestamp, rightHandIndex, jointsPos[rightHandIndex]);
                            gestureData.progress = 0.5f;
                    ...
                    case 1:
                        if ((timestamp - gestureData.timestamp) < 1.2f)
                        {
                            ...
                            // bool isMotionZ = jointsPos [rightHandIndex].z <= (jointsPos [hipCenterIndex].z + jointsPos [leftHipIndex].z + jointsPos [rightHipIndex].z) / 3.0f ;
                            bool isMotionY = jointsPos[rightHandIndex].y <= jointsPos[SpineIndex].y
                                        && jointsTracked[rightHandIndex] && jointsTracked[SpineIndex];
                            bool isMotionX = jointsPos[rightHandIndex].x >= jointsPos[leftShoulderIndex].x
                                        && jointsTracked[rightHandIndex] && jointsTracked[leftShoulderIndex];

                            bool isInPose = isMotionY && isMotionX;
                            // && jointsPos [rightHandIndex].y >= ((jointsPos [hipCenterIndex].y + jointsPos [shoulderCenterIndex].y) / 3.0f);

                            string condition = string.Format("{0} {1}", isMotionY, isMotionX);
                            Debug.Log("SwingTopdown 계산 : " + condition);

                            if (isInPose)
                            {
                                Debug.Log("SwingTopdown 완료");
                                Vector3 jointPos = jointsPos[gestureData.joint];
                                CheckPoseComplete(ref gestureData, timestamp, jointPos, isInPose, 0f);
                            }
                            else {
